...
sector_data <- x %>%
  #select(fund_symbol, sector_basic_materials : sector_technology, fund_return_1year) %>%
  select(fund_symbol, sector_basic_materials : sector_financial_services, sector_technology, fund_return_1year) %>%
  pivot_longer(names_to = "sector",
               values_to = "ratio",
               cols = c(-fund_symbol, -fund_return_1year)) %>%
...
# maybe filter(ratio >= 25), so we only see stocks that have a majority in one sector
```
...
x %>%
  ...
  ggplot(mapping = aes(x = fct_reorder(investment_type, avg_return), y = avg_return)) +
    geom_col(fill = "LightBlue") +
    theme_classic() +
    labs(title = "Investment Risk vs. Annual Returns",
